...
# -*- coding: utf-8 -*-
"""
多目标 DPO 加权和：亲和力 / 稳定性 / 溶解度 固定权重 1:1:1（λ 均为 1.0）。
训练逻辑与 utils/dpo/train_DPO_multi_objective.py 一致；权重保存到 logs_weighted_sum/。
未设置 CUDA_VISIBLE_DEVICES 时默认仅暴露物理 2 号 GPU。
"""
...
import json, argparse, copy, math, random
...
def main():
    ap = argparse.ArgumentParser(description="多目标 DPO 加权和 1:1:1（λ_aff=λ_stab=λ_sol=1）")
    ...
    ap.add_argument("--jsonl", default=None, help="兼容：只给 --jsonl 时视为亲和力数据路径")
    ...
    ap.add_argument("--save_dir", type=str, default="/root/autodl-tmp/Peptide_3D/logs_weighted_sum")

    ap.add_argument("--epochs", type=int, default=3)
    ap.add_argument("--batch_size", type=int, default=1)
    ap.add_argument("--num_workers", type=int, default=1)

    # 固定 1:1:1；可选归一化为各 1/3
    ap.add_argument("--normalize_lambda", action="store_true",
                    help="将三个任务的 λ 归一化使和为 1（即各 1/3）")

    ap.add_argument(
        "--gpu",
        type=int,
        default=0,
        help="进程内 CUDA 序号；若已通过 CUDA_VISIBLE_DEVICES 只暴露一张卡，则一般为 0",
    )
    ...
    # 固定权重 1:1:1
    lambda_aff = lambda_stab = lambda_sol = 1.0
    ...
    def _try_add(name, path, lam):
        if path and os.path.exists(path) and lam > 0:
            ...
            if len(ds) > 0:
                ...
                return
    _try_add("aff", args.aff_jsonl, lambda_aff)
    _try_add("stab", args.stab_jsonl, lambda_stab)
    _try_add("sol", args.sol_jsonl, lambda_sol)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        pbar = tqdm(range(steps_max), desc=f"DPO weighted 1:1:1 epoch {epoch}", dynamic_ncols=True)
        ...
        if args.save_every_epoch:
            ckpt = os.path.join(args.save_dir, f"policy_dpo_weighted_sum_epoch{epoch}_loss_{avg:.4f}.pth")
            torch.save(policy.state_dict(), ckpt)
        if avg < best:
            best = avg
            torch.save(policy.state_dict(), os.path.join(args.save_dir, "policy_dpo_weighted_sum_best.pth"))

    print(f"[FIN] Weighted-sum multi-objective DPO (1:1:1) done. Best={best:.4f}, saved under {args.save_dir}")
...
nohup python /root/autodl-tmp/Peptide_3D/results/3_Pareto_improved/train_DPO_weighted_sum.py \
  --use_amp \
  --epochs 5 
  > /root/autodl-tmp/Peptide_3D/results/3_Pareto_improved/weighted_sum.log 2>&1 &
