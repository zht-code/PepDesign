...
# -*- coding: utf-8 -*-
"""
稳定性单任务 DPO：仅使用稳定性偏好对 jsonl，不做 aff/sol 多目标混合。
数据格式与 utils/dpo/train_DPO_multi_objective.py 中 stab 任务一致（chosen/rejected + score）。
权重默认保存到 logs_stab_only/。
未设置 CUDA_VISIBLE_DEVICES 时，默认仅暴露物理 1 号 GPU（与 affinity_only 脚本中 0 号卡对称）。
"""
...
class DPOPairsDataset(Dataset):
    """
    稳定性偏好对 jsonl；Δreward 与多目标脚本一致：优先 score.R，否则 hdock。
    """
...
def main():
    ap = argparse.ArgumentParser(description="稳定性单任务 DPO")
    ap.add_argument(
        "--jsonl",
        type=str,
        default="/root/autodl-tmp/Peptide_3D/utils/dpo/stability_pairs.jsonl",
        help="稳定性偏好对 jsonl",
    )
    ap.add_argument(
        "--init_ckpt",
    ...
    ap.add_argument(
        "--save_dir",
        type=str,
        default="/root/autodl-tmp/Peptide_3D/logs_stab_only",
    )
    ...
    if not os.path.exists(args.jsonl):
        raise RuntimeError(f"找不到稳定性 jsonl: {args.jsonl}")

    ds = DPOPairsDataset(args.jsonl, max_len=args.max_len, max_receptor_len=args.max_receptor_len)
    if len(ds) == 0:
        raise RuntimeError("稳定性数据集为空，请检查 jsonl 与 pdb 路径。")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(range(steps_per_epoch), desc=f"DPO stability epoch {epoch}", dynamic_ncols=True)
        micro = 0
        ...
        if args.save_every_epoch:
            ckpt = os.path.join(
                args.save_dir, f"policy_dpo_stability_epoch{epoch}_loss_{avg:.4f}.pth"
            )
        ...
        if avg < best:
            ...
            torch.save(
                policy.state_dict(),
                os.path.join(args.save_dir, "policy_dpo_stability_best.pth"),
            )

    print(f"[FIN] DPO stability-only done. Best={best:.4f}, saved under {args.save_dir}")
...
nohup python /root/autodl-tmp/Peptide_3D/results/3_Pareto_improved/train_DPO_stability_only.py \
  --jsonl /root/autodl-tmp/Peptide_3D/utils/dpo/stability_pairs.jsonl \
  --use_amp \
  --epochs 5
  > /root/autodl-tmp/Peptide_3D/results/3_Pareto_improved/stability_only.log 2>&1 &
