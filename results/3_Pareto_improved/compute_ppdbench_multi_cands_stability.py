...
# -*- coding: utf-8 -*-
"""
对 PPDbench 各靶点 <target>/multi_cands/ 下「你的方法」生成的多肽计算稳定性（FoldX），
与 compute_ppdbench_generated_stability.py 使用相同计算逻辑。

每个靶点计算的条数由 --top-k 控制（默认 3）：
  - top-k <= 0：该靶点 multi_cands 下全部 pep_*.pdb
  - top-k > 0：先按 HDOCK 升序取，不足则用 pep_*.pdb 补足，至多 top-k 条

结果写入：results/3_Pareto_improved/ppdbench_stability_multi_cands.json
...
from compute_ppdbench_generated_stability import foldx_stability_score_single  # noqa: E402

OUT_JSON = "ppdbench_stability_multi_cands.json"
...
def main():
    ap = argparse.ArgumentParser(description="PPDbench multi_cands top-K stability (FoldX)")
    ...
    ap.add_argument(
        ...
        type=int,
        default=3,
        help="每靶点条数；<=0 表示全部 pep_*.pdb",
    )
    ap.add_argument("--workers", type=int, default=72)
    ap.add_argument("--foldx-bin", type=str, default="/root/autodl-tmp/foldx_20270131")
    ap.add_argument("--workdir-root", type=str, default="/root/autodl-tmp/tmp_foldx_eval_multi_cands")
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        pbar = tqdm(total=len(pending), desc="multi_cands stab", unit="pep")
...
python /root/autodl-tmp/Peptide_3D/results/3_Pareto_improved/compute_ppdbench_multi_cands_stability.py --workers 72 --skip-existing
