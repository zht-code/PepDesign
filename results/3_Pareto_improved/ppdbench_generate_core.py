"""
PPDbench 多肽 PDB 生成核心逻辑（由四个入口脚本调用）。
- 每个靶点目录需含 receptor.pdb（不使用首字母排序的 *.pdb，避免误选 peptide.pdb）。
- 模型与训练一致：models_dpo.ProteinPeptideModel + 对应 checkpoint。
"""
...
# ---------- 其他工具 ----------
def find_ppdbench_receptors(root: str):
    """PPDbench：每个 PDB ID 子目录下使用 receptor.pdb（共 133 个靶点）。"""
    pairs = []
...
def worker(gpu_id: int, prot_shard: list, cfg: dict):
    ...
    """
    base = int(cfg.get("base_cuda_device", 0))
    dev_idx = base + int(gpu_id)
    device = torch.device(f"cuda:{dev_idx}" if torch.cuda.is_available() else "cpu")
    if device.type == "cuda":
    ...
    with torch.no_grad():
        for prot_dir, pdb_path in it:
            ...
            out_dir = os.path.join(prot_dir, cfg.get("out_subdir", "generated_peptides"))
...
# ------------------ 主控 ------------------
def run_ppdbench_with_defaults(
    *,
    default_ckpt: str,
    ...
    p = argparse.ArgumentParser(
        description="PPDbench：按靶点生成多肽 PDB（逻辑同 test_data_generate_top10.py）"
    )
    p.add_argument("--bench-root", default="/root/autodl-tmp/PPDbench")
    p.add_argument("--ckpt", type=str, default=default_ckpt, help="ProteinPeptideModel 权重（models_DPO）")
    p.add_argument("--out-subdir", type=str, default=default_out_subdir,
                   help="写入各靶点目录下的子目录名，如 generated_dpo_affinity_only")
    p.add_argument("--num-per-target", type=int, default=3, help="每个靶点保留的多肽条数")
    p.add_argument("--top-k", type=int, default=12)
    p.add_argument("--max-len", type=int, default=30)
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--want-gpus", type=int, default=1)
    p.add_argument(
        "--gpu",
        type=int,
        default=0,
        help="使用的 CUDA 设备起始编号：单卡时只用 cuda:gpu；多卡时依次使用 cuda:gpu, cuda:gpu+1, ...（也可用环境变量 CUDA_VISIBLE_DEVICES 再设 --gpu 0）",
    )
    ...
    prot_list = find_ppdbench_receptors(args.bench_root)
    total = len(prot_list)
    print(f"Found {total} targets (receptor.pdb) under {args.bench_root}.")
    ...
    if avail == 0:
        ...
        world_size = min(args.want_gpus, avail)
        need = args.gpu + world_size
        if need > avail:
            print(
                f"[ERROR] 需要 cuda:{args.gpu} .. cuda:{need - 1}，但当前可见 GPU 仅 {avail} 张（device_count）."
            )
            return 1
    ...
    cfg = dict(
        ckpt_path=args.ckpt,
        num_per_protein=args.num_per_target,
        top_k=args.top_k,
    ...
    if world_size == 1:
        ...
        for rank in range(world_size):
            pr = ctx.Process(target=worker, args=(rank, shards[rank], cfg), daemon=False)
            pr.start()
            procs.append(pr)
        for pr in procs:
            pr.join()
...
def main():
    """直接运行本文件时的占位：请用 generate_ppdbench_*.py 入口。"""
    print("请使用同目录下的 generate_ppdbench_DPO_affinity_only.py 等入口脚本，"
          "或调用 run_ppdbench_with_defaults(default_ckpt=..., default_out_subdir=...).")
    return 1
