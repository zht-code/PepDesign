...
import json
NEIGHBORS_JSON_PATH = "/root/autodl-tmp/Peptide_3D/utils/Data_augmentation/ot_top10_neighbors_4.json"
...
def load_all_files(root: str):
    ...
    for p in paths:
        ...
        tmp = torch.load(p, map_location="cuda:0")  # 如需 GPU 可换成 map_location="cuda:0"
        # 简单校验 key 是否存在
...
def main(root):
    ...
    # 外层：遍历每个蛋白
    outer_bar = tqdm(range(3*p+1, 4*p), desc="蛋白遍历", unit="protein")
    for i in outer_bar:
