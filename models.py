...
class ProteinPeptideModel(nn.Module):
    ...
    def freeze_strategy(self, stage: str = "stage1", n_dec_last: int = 4, n_enc_last: int = 2):
        ...
        # --- NEW: interface head q/k ---
        if hasattr(self.esmc, "iface_q"):
            self._set_requires_grad(self.esmc.iface_q, True)
        if hasattr(self.esmc, "iface_k"):
            self._set_requires_grad(self.esmc.iface_k, True)
    ...
    def build_param_groups(self, base_lr: float = 1e-5):
        ...
        # ESMC heads (+ interface q/k)
        for m in [self.esmc.sequence_head, self.esmc.structure_head,
                getattr(self.esmc, "iface_q", None), getattr(self.esmc, "iface_k", None)]:
            if m is None: 
                continue
            for p in m.parameters():
                if p.requires_grad: 
    ...
    def forward(self, batch ,mode='train'):
        ...
        if mode == 'train':
            ...
            interface_logits = getattr(generated_peptide, "interface_logits", None)
            return sequence_logits, structure_logits, interface_logits
    ...
    @torch.no_grad()
    def last_attn_from_pdb(self, pdb_path: str) -> tuple[torch.Tensor, torch.Tensor]:
        device = self.device if hasattr(self, "device") else next(self.parameters()).device
        ...
        if hasattr(self, "protein_tokenizer"):
            tokens = self.esmc._tokenize([prot_seq])
            pad_id = self.protein_tokenizer.pad_token_id
        else:
            tokens = self.esmc._tokenize([prot_seq])
            pad_id = self.esmc.tokenizer.pad_token_id
        ...
        def trans_hook(module, inp, out):
            # 这里 out 就是 transformer 的输出，一般是 [B, L, 1536]
            last_hidden["x"] = out

        handle = self.esm3_model.transformer.register_forward_hook(trans_hook)

        # 一次 forward 就够了
        _ = self.esm3_model(sequence_tokens=tokens)

        handle.remove()

        if "x" not in last_hidden:
            raise RuntimeError("未捕获到 transformer 输出，检查 hook 注册位置。")

        hidden = last_hidden["x"]    # [1, Lp, 1536]

        encoder_embeddings = hidden
        dtype_dec = next(self.esmc.parameters()).dtype
        encoder_embeddings = encoder_embeddings[0].to(device=device, dtype=dtype_dec)
